...
let latestHandY = null;

window.addEventListener('handPosition', (evt) => {
  latestHandY = evt.detail.mappedY;
});
...
var Game = {
  ...
  update: function () {
    if (!this.over) {
      if (latestHandY !== null) {
        const pongHeight    = this.canvas.height;
        const paddleHeight  = this.player.height;
        let newY = latestHandY * pongHeight - paddleHeight / 2;
        if (newY < 0) newY = 0;
    ...
    if (this.player.score === rounds[this.round]) {
      if (!rounds[this.round + 1]) {
        this.over = true;
        setTimeout(() => Pong.endGameMenu('Winner!'), 1000);
      } else {
      ...
      this.over = true;
      setTimeout(() => Pong.endGameMenu('Game Over!'), 1000);
    }
  },
  ...
  _generateRoundColor: function () {
    ...
    return newColor === this.color ? Pong._generateRoundColor() : newColor;
  }
};
